...
function the_bootstrap_setup() {
	...
	load_theme_textdomain( 'uc-customized', get_template_directory() . '/lang' );
	...
	register_nav_menus( array(
		'primary'		=>	__( 'Main Navigation', 'uc-customized' ),
		'header-menu'  	=>	__( 'Header Menu', 'uc-customized' ),
		'footer-menu' 	=>	__( 'Footer Menu', 'uc-customized' )
	) );
...
/**
 * Returns the options object for uCertify Customized.
 *
 * @author	Automattic
...
/**
 * Returns the default options for uCertify Customized.
 *
 * @author	Automattic
 * @since	1.3.0 - 06.04.2012
 *
...
/**
 * Adds uCertify Customized layout classes to the array of body classes.
 *
 * @author	WordPress.org
 * @since	1.3.0 - 06.04.2012
 *
...
function the_bootstrap_widgets_init() {
	...
	register_sidebar( array(
		'name'			=>	__( 'Main Sidebar', 'uc-customized' ),
		'id'			=>	'main',
	...
	register_sidebar( array(
		'name'			=>	__( 'Image Sidebar', 'uc-customized' ),
		'description'	=>	__( 'Shown on image attachment pages.', 'uc-customized' ),
		'id'			=>	'image',
		'before_widget'	=>	'<aside id="%1$s" class="widget well %2$s">',
		'after_widget'	=>	'</aside>',
		'before_title'	=>	'<h2 class="widget-title">',
	...
	include_once( 'inc/uc-customized-image-meta-widget.php' );
	register_widget( 'The_Bootstrap_Image_Meta_Widget' );
	
	include_once( 'inc/uc-customized-gallery-widget.php' );
...
function the_bootstrap_register_scripts_styles() {
	...
	if ( ! is_admin() ) {
		...
		wp_register_script(
			'uc-customized',
			get_template_directory_uri() . "/js/uc-customized{$suffix}.js",
			array('tw-bootstrap'),
			$theme_version,
			true
		);
...
function the_bootstrap_print_scripts() {
	wp_enqueue_script( 'uc-customized' );
}
...
function the_bootstrap_print_styles() {
	if ( is_child_theme() ) {
		wp_enqueue_style( 'uc-customized-child', get_stylesheet_uri(), array( 'uc-customized' ) );
	} else {
		wp_enqueue_style( 'uc-customized' );
	}
	...
	if ( 'static' != the_bootstrap_options()->navbar_position ) {
		...
		if ( function_exists( 'wp_add_inline_style' ) )
			wp_add_inline_style( 'uc-customized', $css );
		else
...
function the_bootstrap_credits() {
	printf(
		'<span class="credits alignleft">' . __( '&copy; %1$s <a href="%2$s">%3$s</a>, all rights reserved.', 'uc-customized' ) . '</span>',
		date( 'Y' ),
		home_url( '/' ),
		get_bloginfo( 'name' )
	);
...
function the_bootstrap_continue_reading_link() {
	return ' <a href="'. esc_url( get_permalink() ) . '">' . __( 'Continue reading <span class="meta-nav">&rarr;</span>', 'uc-customized' ) . '</a>';
}
...
function the_bootstrap_comments_list() {
	if ( post_password_required() ) : ?>
		<div id="comments">
			<p class="nopassword"><?php _e( 'This post is password protected. Enter the password to view any comments.', 'uc-customized' ); ?></p>
		</div><!-- #comments -->
		<?php
		return;
	endif;
	...
	if ( have_comments() ) : ?>
		<div id="comments">
			<h2 id="comments-title">
				<?php printf( _n( 'One thought on &ldquo;%2$s&rdquo;', '%1$s thoughts on &ldquo;%2$s&rdquo;', get_comments_number(), 'uc-customized' ),
...
function the_bootstrap_comments_closed() {
	if ( ! is_page() AND post_type_supports( get_post_type(), 'comments' ) ) : ?>
		<p class="nocomments"><?php _e( 'Comments are closed.', 'uc-customized' ); ?></p>
	<?php endif;
}
...
function the_bootstrap_comment_form_defaults( $defaults ) {
	return wp_parse_args( array(
		'comment_field'			=>	'<div class="comment-form-comment control-group"><label class="control-label" for="comment">' . _x( 'Comment', 'noun', 'uc-customized' ) . '</label><div class="controls"><textarea class="span7" id="comment" name="comment" rows="8" aria-required="true"></textarea></div></div>',
		'comment_notes_before'	=>	'',
		'comment_notes_after'	=>	'<div class="form-allowed-tags control-group"><label class="control-label">' . sprintf( __( 'You may use these <abbr title="HyperText Markup Language">HTML</abbr> tags and attributes: %s', 'uc-customized' ), '</label><div class="controls"><pre>' . allowed_tags() . '</pre></div>' ) . '</div>
									 <div class="form-actions">',
		'title_reply'			=>	'<legend>' . __( 'Leave a reply', 'uc-customized' ) . '</legend>',
		'title_reply_to'		=>	'<legend>' . __( 'Leave a reply to %s', 'uc-customized' ). '</legend>',
		'must_log_in'			=>	'<div class="must-log-in control-group controls">' . sprintf( __( 'You must be <a href="%s">logged in</a> to post a comment.', 'uc-customized' ), wp_login_url( apply_filters( 'the_permalink', get_permalink( get_the_ID() ) ) ) ) . '</div>',
		'logged_in_as'			=>	'<div class="logged-in-as control-group controls">' . sprintf( __( 'Logged in as <a href="%1$s">%2$s</a>. <a href="%3$s" title="Log out of this account">Log out?</a>', 'uc-customized' ), admin_url( 'profile.php' ), wp_get_current_user()->display_name, wp_logout_url( apply_filters( 'the_permalink', get_permalink( get_the_ID() ) ) ) ) . '</div>',
	), $defaults );
}
...
function the_bootstrap_comment( $comment, $args, $depth ) {
	...
	if ( 'pingback' == $comment->comment_type OR 'trackback' == $comment->comment_type ) : ?>
		...
		<li id="li-comment-<?php comment_ID(); ?>" <?php comment_class(); ?>>
			<p class="row">
				<strong class="ping-label span1"><?php _e( 'Pingback:', 'uc-customized' ); ?></strong>
				<span class="span7"><?php comment_author_link(); edit_comment_link( __( 'Edit', 'uc-customized' ), '<span class="sep">&nbsp;</span><span class="edit-link label label-success">', '</span>' ); ?></span>
			</p>
			...
			<article id="comment-<?php comment_ID(); ?>" class="comment row">
				...
				<footer class="comment-meta span<?php echo $span; ?>">
					<p class="comment-author vcard">
						<?php
							...
							printf( __( '%1$s <span class="says">said</span> on %2$s:', 'uc-customized' ),
								sprintf( '<span class="fn">%s</span>', get_comment_author_link() ),
								sprintf( '<a href="%1$s"><time pubdate datetime="%2$s">%3$s</time></a>',
									esc_url( get_comment_link( $comment->comment_ID ) ),
									get_comment_time( 'c' ),
									/* translators: 1: date, 2: time */
									sprintf( __( '%1$s at %2$s', 'uc-customized' ), get_comment_date(), get_comment_time() )
								)
							);
							edit_comment_link( __( 'Edit', 'uc-customized' ), '<span class="sep">&nbsp;</span><span class="edit-link label label-success">', '</span>' ); ?>
					</p><!-- .comment-author .vcard -->
	
					<?php if ( ! $comment->comment_approved ) : ?>
					<div class="comment-awaiting-moderation alert alert-info"><em><?php _e( 'Your comment is awaiting moderation.', 'uc-customized' ); ?></em></div>
					<?php endif; ?>
				...
				<div class="comment-content span<?php echo $span; ?>">
					...
					comment_reply_link( array_merge( $args, array(
						'reply_text'	=>	__( 'Reply <span>&darr;</span>', 'uc-customized' ),
						'depth'			=>	$depth,
...
function the_bootstrap_comment_form_field_author( $html ) {
	...
	return	'<div class="comment-form-author control-group">
				<label for="author" class="control-label">' . __( 'Name', 'uc-customized' ) . '</label>
				<div class="controls">
					<input id="author" name="author" type="text" value="' . esc_attr(  $commenter['comment_author'] ) . '" size="30"' . $aria_req . ' />
					' . ( $req ? '<p class="help-inline"><span class="required">' . __('required', 'uc-customized') . '</span></p>' : '' ) . '
				</div>
			</div>';
}
...
function the_bootstrap_comment_form_field_email( $html ) {
	...
	return	'<div class="comment-form-email control-group">
				<label for="email" class="control-label">' . __( 'Email', 'uc-customized' ) . '</label>
				<div class="controls">
					<input id="email" name="email" type="email" value="' . esc_attr(  $commenter['comment_author_email'] ) . '" size="30"' . $aria_req . ' />
					<p class="help-inline">' . ( $req ? '<span class="required">' . __('required', 'uc-customized') . '</span>, ' : '' ) . __( 'will not be published', 'uc-customized' ) . '</p>
				</div>
			</div>';
}
...
function the_bootstrap_comment_form_field_url( $html ) {
	...
	return	'<div class="comment-form-url control-group">
				<label for="url" class="control-label">' . __( 'Website', 'uc-customized' ) . '</label>
				<div class="controls">
...
function the_bootstrap_post_gallery( $content, $attr ) {
	...
	foreach ( $attachments as $id => $attachment ) {
		...
		if ( $captiontag AND ( 0 < $comments OR trim( $attachment->post_excerpt ) ) ) {
			$comments	=	( 0 < $comments ) ? sprintf( _n('%d comment', '%d comments', $comments, 'uc-customized'), $comments ) : '';
 ...
 * @param	string	$form
 *
 * @return	string	uCertify Customized password form
 */
function the_bootstrap_the_password_form( $form ) {
	return '<form class="post-password-form form-horizontal" action="' . home_url( 'wp-pass.php' ) . '" method="post"><legend>'. __( 'This post is password protected. To view it please enter your password below:', 'uc-customized' ) . '</legend><div class="control-group"><label class="control-label" for="post-password-' . get_the_ID() . '">' . __( 'Password:', 'uc-customized' ) .'</label><div class="controls"><input name="post_password" id="post-password-' . get_the_ID() . '" type="password" size="20" /></div></div><div class="form-actions"><button type="submit" class="post-password-submit submit btn btn-primary">' . __( 'Submit', 'uc-customized' ) . '</button></div></form>';
}
 ...
 * @access	private
 *
 * @return	string	uCertify Customized version
 */
...
/* Location: ./wp-content/themes/uc-customized/functions.php */