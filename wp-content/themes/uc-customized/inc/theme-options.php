...
/** theme-options.php
 * 
 * uCertify Customized Theme Options
 *
...
function the_bootstrap_theme_options_add_page() {
	$theme_page = add_theme_page(
		__( 'Theme Options', 'uc-customized' ),		// Name of page
		__( 'Theme Options', 'uc-customized' ),		// Label in menu
...
function the_bootstrap_admin_enqueue_scripts( $hook_suffix ) {
	wp_enqueue_style( 'uc-customized-theme-options', get_template_directory_uri() . '/css/theme-options.css', false, _the_bootstrap_version() );
}
...
function the_bootstrap_theme_options_init() {
	...
	add_settings_field( 'layout', __( 'Default Layout', 'uc-customized' ), 'the_bootstrap_settings_field_layout', 'theme_options', 'general' );
	add_settings_field( 'navbar', __( 'Navigation Bar', 'uc-customized' ), 'the_bootstrap_settings_field_checkbox', 'theme_options', 'general', array(
		(object) array(
			'name'			=>	'navbar_site_name',
			'value'			=>	the_bootstrap_options()->navbar_site_name,
			'description'	=>	__( 'Add site name to navigation bar.', 'uc-customized' )
		),
		(object) array(
			'name'			=>	'navbar_searchform',
			'value'			=>	the_bootstrap_options()->navbar_searchform,
			'description'	=>	__( 'Add searchform to navigation bar.', 'uc-customized' )
		)
	) );
	add_settings_field( 'navbar-position', __( 'Navigation Bar Position', 'uc-customized' ), 'the_bootstrap_settings_field_radio', 'theme_options', 'general', array(
		'name'		=>	'navbar_position',
		'options'	=>	array(
			(object) array(
				'value'			=>	'static',
				'description'	=>	__( 'Static.', 'uc-customized' )
			),
			(object) array(
				'value'			=>	'navbar-fixed-top',
				'description'	=>	__( 'Fixed on top.', 'uc-customized' )
			),
			(object) array(
				'value'			=>	'navbar-fixed-bottom',
				'description'	=>	__( 'Fixed at bottom.', 'uc-customized' )
			),
		)
	) );
}
...
function the_bootstrap_admin_bar_menu( $wp_admin_bar ) {
	if ( current_user_can( 'edit_theme_options' ) AND is_admin_bar_showing() ) {
		$wp_admin_bar->add_menu( array(
			'title'		=>	__( 'Theme Options', 'uc-customized' ),
			'href'		=>	add_query_arg( array( 'page' => 'theme_options' ), admin_url( 'themes.php' ) ),
			'parent'	=>	'appearance',
			'id'		=>	'uc-customized-theme-options',
		) );
	}
}
...
function the_bootstrap_layouts() {
	$layout_options	=	array(
		'content-sidebar'	=>	array(
			'label'		=>	__( 'Content on left', 'uc-customized' ),
			'thumbnail'	=>	get_template_directory_uri() . '/img/content-sidebar.png',
		),
		'sidebar-content'	=>	array(
			'label'		=>	__( 'Content on right', 'uc-customized' ),
...
/**
 * Renders the Settings page for uCertify Customized.
 *
 * @author	Automattic
 * @since	1.3.0 - 06.04.2012
 * 
...
function the_bootstrap_theme_options_render_page() {
	...
	<div class="wrap">
		<?php screen_icon(); ?>
		<h2><?php esc_html_e( 'uCertify Customized Theme Options', 'uc-customized' ); ?></h2>
		<?php settings_errors(); ?>
...
function the_bootstrap_theme_options_validate( $input ) {
	...
	if ( ! get_settings_errors() ) {
		add_settings_error( 'uc-customized-options', 'settings_updated', sprintf( __( 'Settings saved. <a href="%s">Visit your site</a> to see how it looks.', 'uc-customized' ), home_url( '/' ) ), 'updated' );
	}
...
function the_bootstrap_donate_box() {
	...
	<div id="formatdiv" class="postbox">
		<h3 class="hndle"><span><?php esc_html_e( 'Help spread the word!', 'uc-customized' ); ?></span></h3>
		<div class="inside">
			<p><strong><?php printf( _x( 'Want to help make this Theme even better? All donations are used to improve %1$s, so donate $20, $50 or $100 now!', 'Plugin Name', 'uc-customized' ), esc_html('uCertify Customized ') ); ?></strong></p>
			...
			<p><?php _e( 'Or you could:', 'uc-customized' ); ?></p>
			<ul>
				<li><a href="http://wordpress.org/extend/themes/uc-customized"><?php _e( 'Rate the Theme 5&#9733; on WordPress.org', 'uc-customized' ); ?></a></li>
				<li><a href="http://en.wp.obenland.it/uc-customized/"><?php _e( 'Blog about it &amp; link to the Theme page', 'uc-customized' ); ?></a></li>
			</ul>
		</div>
	</div>
	<?php
...
function the_bootstrap_feed_box() {
	...
	?>
	<div id="formatdiv" class="postbox">
		<h3 class="hndle"><span><?php esc_html_e( 'News from Konstantin', 'uc-customized' ); ?></span></h3>
		<div class="inside">
			<ul>
			<?php if ( ! $rss_items ) : ?>
			<li><?php _e( 'No news items, feed might be broken...', 'uc-customized' ); ?></li>
			<?php else :
			foreach ( $rss_items as $item ) :
				$url = preg_replace( '/#.*/', '#utm_source=wordpress&utm_medium=sidebannerpostbox&utm_term=rssitem&utm_campaign=uc-customized',  $item->get_permalink() ); ?>
			<li><a class="rsswidget" href="<?php echo esc_url( $url ); ?>"><?php echo esc_html( $item->get_title() ); ?></a></li>
			<?php endforeach; endif; ?>
				<li class="twitter"><a href="http://twitter.com/obenland"><?php _e( 'Follow Konstantin on Twitter', 'uc-customized' ); ?></a></li>
				<li class="rss"><a href="http://en.wp.obenland.it/feed/"><?php _e( 'Subscribe via RSS', 'uc-customized' ); ?></a></li>
			</ul>
		</div>
	</div>
	<?php
...
/* Location: ./wp-content/themes/uc-customized/inc/theme-options.php */