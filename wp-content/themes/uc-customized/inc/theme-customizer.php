...
function the_bootstrap_customize_register( $wp_customize ) {
	...
	$wp_customize->add_section( 'the_bootstrap_theme_layout', array(
		'title'		=>	__( 'Layout', 'uc-customized' ),
		'priority'	=>	99,
	) );
	$wp_customize->add_section( 'the_bootstrap_navbar_options', array(
			'title'		=>	__( 'Navbar Options', 'uc-customized' ),
			'priority'	=>	101,
	) );
	...
	$wp_customize->add_control( 'the_bootstrap_theme_layout', array(
		'label'		=>	__( 'Default Layout', 'uc-customized' ),
		'section'	=>	'the_bootstrap_theme_layout',
		'settings'	=>	'the_bootstrap_theme_options[theme_layout]',
		'type'		=>	'radio',
		'choices'	=>	array(
			'content-sidebar'	=>	__( 'Content on left', 'uc-customized' ),
			'sidebar-content'	=>	__( 'Content on right', 'uc-customized' )
		),
	) );
	
	// Sitename in Navbar
	$wp_customize->add_control( 'the_bootstrap_navbar_site_name', array(
		'label'		=>	__( 'Add site name to navigation bar.', 'uc-customized' ),
	...
	$wp_customize->add_control( 'the_bootstrap_navbar_searchform', array(
		'label'		=>	__( 'Add searchform to navigation bar.', 'uc-customized' ),
		'section'	=>	'the_bootstrap_navbar_options',
	...
	$wp_customize->add_control( 'the_bootstrap_navbar_inverse', array(
		'label'		=>	__( 'Use inverse color on navigation bar.', 'uc-customized' ),
		'section'	=>	'the_bootstrap_navbar_options',
	...
	$wp_customize->add_control( 'the_bootstrap_navbar_position', array(
		'label'		=>	__( 'Navigation Bar Position', 'uc-customized' ),
		'section'	=>	'the_bootstrap_navbar_options',
		'settings'	=>	'the_bootstrap_theme_options[navbar_position]',
		'type'		=>	'radio',
		'choices'	=>	array(
			'static'				=>	__( 'Static.', 'uc-customized' ),
			'navbar-fixed-top'		=>	__( 'Fixed on top.', 'uc-customized' ),
			'navbar-fixed-bottom'	=>	__( 'Fixed at bottom.', 'uc-customized' ),
		),
	) );
}
...
function the_bootstrap_customize_enqueue_scripts() {
	wp_enqueue_script( 'uc-customized-customize', get_template_directory_uri() . '/js/theme-customizer.js', array( 'customize-preview' ), _the_bootstrap_version(), true );
	wp_localize_script( 'uc-customized-customize', 'the_bootstrap_customize', array(
...
/* Location: ./wp-content/themes/uc-customized/inc/theme-customizer.php */