...
function the_bootstrap_comment_nav() {
	...
	<nav class="comment-nav well">
		<h1 class="assistive-text"><?php _e( 'Comment navigation', 'uc-customized' ); ?></h1>
		<div class="nav-previous alignleft"><?php next_comments_link( __( '&larr; Newer Comments', 'uc-customized' ) ); ?></div>
		<div class="nav-next alignright"><?php previous_comments_link( __( 'Older Comments &rarr;', 'uc-customized' ) ); ?></div>
	</nav>
...
function the_bootstrap_posted_on() {
	printf( __( '<span class="sep">Posted on </span><a href="%1$s" title="%2$s" rel="bookmark"><time class="entry-date" datetime="%3$s" pubdate>%4$s</time></a><span class="by-author"> <span class="sep"> by </span> <span class="author vcard"><a class="url fn n" href="%5$s" title="%6$s" rel="author">%7$s</a></span></span>', 'uc-customized' ),
			esc_url( get_permalink() ),
			esc_attr( get_the_time() ),
			esc_attr( get_the_date( 'c' ) ),
			esc_html( get_the_date() ),
			esc_url( get_author_posts_url( get_the_author_meta( 'ID' ) ) ),
			esc_attr( sprintf( __( 'View all posts by %s', 'uc-customized' ), get_the_author() ) ),
			get_the_author()
	);
	if ( comments_open() AND ! post_password_required() ) { ?>
		<span class="sep"> | </span>
		<span class="comments-link">
			<?php comments_popup_link( '<span class="leave-reply">' . __( 'Leave a reply', 'uc-customized' ) . '</span>', __( '<strong>1</strong> Reply', 'uc-customized' ), __( '<strong>%</strong> Replies', 'uc-customized' ) ); ?>
		</span>
		<?php
	}
	edit_post_link( __( 'Edit', 'uc-customized' ), '<span class="sep">&nbsp;</span><span class="edit-link label">', '</span>' );
}
endif;
 ...
 *
 * It's basically the wp_link_pages() function, altered to fit to uCertify Customized
...
function the_bootstrap_link_pages( $args = array() ) {
	...
	$defaults = array(
		...
		'nextpagelink'		=> __('Next page', 'uc-customized'),
		'previouspagelink'	=> __('Previous page', 'uc-customized'),
		'pagelink'			=> '%',
		'echo'				=> true
	);
	...
	if ( $multipage ) {
		if ( 'number' == $next_or_number ) {
			$output .= '<nav class="pagination clear"><ul><li><span class="dots">' . __('Pages:', 'uc-customized') . '</span></li>';
...
function the_bootstrap_navbar_searchform( $echo = true ) {
	$searchform = '	<form id="searchform" class="navbar-search pull-right" method="get" action="' . esc_url( home_url( '/' ) ) . '">
						<label for="s" class="assistive-text hidden">' . __( 'Search', 'uc-customized' ) . '</label>
						<input type="search" class="search-query" name="s" id="s" placeholder="' . esc_attr__( 'Search', 'uc-customized' ) . '" />
					</form>';
...
/**
 * Adds uCertify Customized navbar classes
 *
...
/* Location: ./wp-content/themes/uc-customized/inc/template-tags.php */