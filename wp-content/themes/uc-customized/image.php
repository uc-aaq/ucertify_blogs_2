	...
	<div id="content" role="main">
		...
		<nav id="nav-single" class="well clearfix">
			<span class="gallery-link pull-left">
				...
				<?php printf(
					_x( '&laquo; %1$s (%2$s)', 'Post title, amount of images', 'uc-customized' ),
					get_the_title( $post->post_parent ),
					sprintf(
						_nx( '%d image', '%d images', $total_images, 'Amount of images', 'uc-customized' ),
						$total_images
					)
				); ?>
				</a>
			...
			<span class="nav-links pull-right">
				<?php
				edit_post_link( __( 'Edit', 'uc-customized' ), ' <span class="edit-link label">', '</span><span class="sep">&nbsp;</span>' );
				the_bootstrap_comments_link( __( 'Leave a comment', 'uc-customized' ) );
				if ( isset($attachments[$k-1]) )
					echo ' &mdash; <a href="' . get_permalink( $attachments[$k-1]->ID ) . '">' . __( '&laquo; Previous Photo', 'uc-customized' ) . '</a>';
				if ( isset($attachments[$k+1]) )
					echo ' &mdash; <a href="' . get_permalink( $attachments[$k+1]->ID ) . '">' . __( 'Next Photo &raquo;', 'uc-customized' ) . '</a>';
				?>
			</span>
		</nav><!-- #nav-single -->
...
/* Location: ./wp-content/themes/uc-customized/image.php */